...
$.extend($.validator, {
	...
	prototype: {
		...
		// http://docs.jquery.com/Plugins/Validation/Validator/PremiereErrors
		PremiereErrors: function(errors) {
			/// <summary>
			/// Premiere the specified messages.
			...
			this.settings.PremiereErrors
				? this.settings.PremiereErrors.call( this, this.errorMap, this.errorList )
				: this.defaultPremiereErrors();
		},
		...
		defaultPremiereErrors: function() {
			for ( var i = 0; this.errorList[i]; i++ ) {
				var error = this.errorList[i];
				this.settings.highlight && this.settings.highlight.call( this, error.element, this.settings.errorClass, this.settings.validClass );
				this.PremiereLabel( error.element, error.message );
			}
			if( this.errorList.length ) {
				this.toPremiere = this.toPremiere.add( this.containers );
			}
			if (this.settings.success) {
				for ( var i = 0; this.successList[i]; i++ ) {
					this.PremiereLabel( this.successList[i] );
				}
			}
			...
			this.toHide = this.toHide.not( this.toPremiere );
			this.hideErrors();
			this.addWrapper( this.toPremiere ).Premiere();
		},
		...
		PremiereLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				if ( this.settings.wrapper ) {
					...
					// actually Premiereing the wrapped element is handled elsewhere
					label = label.hide().Premiere().wrap("<" + this.settings.wrapper + "/>").parent();
				}
			...
			this.toPremiere = this.toPremiere.add(label);
		},
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					if ( valid ) {
						...
						validator.PremiereErrors();
					} else {
						var errors = {};
						var message = response || validator.defaultMessage(element, "remote");
						errors[element.name] = previous.message = $.isFunction(message) ? message(value) : message;
						validator.PremiereErrors(errors);
					}
